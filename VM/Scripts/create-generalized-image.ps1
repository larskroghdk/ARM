...
$ScriptUri = 'https://github.com/larskroghdk/ARM/blob/main/VM/Scripts/prep-vm.ps1'

# Retrieve the VM to run SysPrep
Write-Host -Object 'Retrieving VM...'
$vm = Get-AzureRmVm -ResourceGroupName $ResourceGroupName -Name $VmName -ErrorAction SilentlyContinue
if ($null -eq $vm) 
{
  Write-Host -Object 'The VM does not exist.'
  exit 1
}
...
Write-Host -Object 'Running sysprep on VM...'
...
{
  Write-Host -Object 'Provisioning extension...'
...
{
  Write-Host -Object 'Executing script...'
  Start-Sleep -Seconds 10
  $status = Get-AzureRmVMDiagnosticsExtension -ResourceGroupName $ResourceGroupName -VmName $VmName -Name $ExtensionName -Status
  $displayStatus = $status.Statuses[0].DisplayStatus
}
...
{
  Write-Host -Object 'Waiting for VM to be stopped...'
  Start-Sleep -Seconds 10
  $vmStatus = Get-AzureRmVm -ResourceGroupName $ResourceGroupName -Name $VmName -Status
  $powerState = $vmStatus.Statuses[1].Code.Split('/')[1]
}
Write-Host -Object 'VM is Stopped.'

# Deallocate the VM
Write-Host -Object 'Deallocating VM...'
...
Write-Host -Object 'Creating generalized image...'
